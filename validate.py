...
ОШИБКИ (блокируют релиз):
  ...
  - ПРЕДУПРЕЖДЕНИЕ «название дублирует слой `pn_*`»: перевод строки дословно
    равен записи слоя. Название живёт в двух местах, и любой из двух
    выключателей по отдельности бесполезен — чтобы увидеть английское, надо
    погасить и категорию, и слой. Лечится тем же приёмом, что названия карты:
    в категории оставить английский оригинал, русскую форму держит слой;
...
# Скобка с текстом внутри в САМОМ оригинале: пояснение игры, а не группа форм.
BRACKET_SRC = re.compile(r'\[[^\[\]]*\]')
# Число замыкает фразу: после него в переводе русских букв уже нет. Так выглядит
# счётчик («Разобрано предметов: %num1%»), где склонять по числу нечего.
NUM_TAIL = re.compile(r'%num\d*%[^А-Яа-яЁё]*$')
...
RU_END = ('ый|ий|ой|ая|яя|ое|ее|ые|ие|ого|его|ому|ему|ым|им|ом|ем|ых|их|ыми|ими|ую|юю'
          '|ов|ев|ей|ам|ям|ами|ями|ах|ях|ия|ию|ии|ием|иям|иями|иях'
          '|а|я|о|е|ы|и|у|ю|й|ь')
...
# «ия|ию|ии|ием|иям|иями|иях» — та же дыра у среднего рода на «-ие». STRIP срезает
# «ие» и оставляет основу «отделен», а склоняется такое слово через «и»:
# «отделения», «отделению», «отделением», «отделении». Без этих окончаний запрет
# ловил только словарную форму — «Астральное отделение» ловилось, «Астрального
# отделения» проходило. Замер по корпусу: три находки, ложных нет.
...
def load_layer_ru(root=None):
    """{английское название: русская форма} — ВЕСЬ слой, без отбора.

    Нужен не для канона, а для поиска дублей: если общая категория повторяет
    ровно то, что отдаёт слой, выключатель имён на этой строке бесполезен.
    """
    out = {}
    for path in layer_files(root):
        try:
            ...
            continue
        for i, r in enumerate(rows):
            if i and len(r) >= 2 and r[0].strip() and r[1].strip():
                out.setdefault(r[0].strip(), r[1].strip())
    return out


LAYER_RU = load_layer_ru()


def layer_duplicate(path, en, ru):
    """Общий словарь дословно повторяет слой — выключатель ничего не изменит.

    Название живёт в двух местах сразу: в своей категории и в `pn_*`. Игрок
    гасит слой — русскую форму отдаёт категория; гасит категорию — форму
    подставляет слой. Чтобы увидеть английское, надо погасить оба, а это не то,
    ради чего слой заводился.

    Лечится тем же приёмом, что и названия карты: в категории оставить
    английский оригинал, русскую форму держать только в слое.

    Сам слой не проверяем — там эта пара и должна лежать.
    """
    if not LAYER_RU or "pn" in path.replace("\\", "/").split("/")[:-1]:
        return None
    if LAYER_RU.get(en.strip()) == ru.strip():
        return ("название дублирует слой `pn_*` — оставь в категории английский "
                "оригинал, русскую форму держит слой (иначе выключатель имён "
                "на этой строке не работает)")
    return None
...
def typography(en, ru):
    ...
    lost = ru.count('«') - ru.count('»')
    # Непарная кавычка — не всегда ошибка: реплика бывает длиннее записи, и в
    # оригинале кавычка открывается, а закрывается уже в следующей строке
    # словаря («"Not going down...without a fight.», «—Rinceword</c>"»).
    # Перевод обязан повторить эту структуру, поэтому ругаемся, только если
    # перекос НЕ такой, как в оригинале: там нечётное число прямых кавычек, а
    # здесь ровно одна лишняя «ёлочка».
    if lost and not (abs(lost) == 1 and en.count('"') % 2 == 1):
...
def check_row(en, ru):
    errs, warns = [], []
    # Кириллица в колонке english — не строка на перевод, а след разъехавшегося
    # CSV: колонки склеились («EN,RU» целиком в первой ячейке) либо запись без
    # кавычек перечитали и разрубили по запятой. Хеш считается от такого огрызка,
    # и в игре запись мертва — совпасть ей не с чем. Остальные правила по ней
    # дают только шум, поэтому дальше не идём.
    m = CYR.search(en)
    if m:
        i = m.start()
        return ["в колонке english кириллица — колонки разъехались: «%s»"
                % en[max(0, i - 20):i + 20]], warns
    ...
    if rs.count('[') != rs.count(']'):
        ...
        for stem, grp in groups:
            ...
            elif BRACKET_SRC.search(strip_known(en)):
                # Скобка с текстом внутри — не группа форм, а пояснение игры
                # («…убежище. [Living World, 1-й сезон…]»), и перевод обязан её
                # сохранить. Ругаться на неё — то же самое, что ругаться на
                # непарную кавычку, стоящую так же в оригинале.
                pass
            else:
        ...
        # Счётчик — не брак: если переводчик перестроил фразу и число уехало в
        # конец («Уничтожено караванов: %num1%»), русская форма от числа больше
        # не зависит — «караванов: 1» и «караванов: 5» одинаково верны, склонять
        # нечего. Отличаем по тому, что после последнего числа русских букв нет.
        if (plural_src and NUM_PH.search(en)
                and not any('|' in g for _, g in groups)
                and not NUM_TAIL.search(ru.strip())):
