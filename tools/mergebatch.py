...
def _weld(base, ours, theirs):
    ...
    def span(ops, i1, i2):
        ...
        У вставки в базе ширины нет (`a1 == a2`), и обычная проверка на
        пересечение отбрасывает её на обеих границах куска: вставка перед
        первым токеном даёт `a2 <= i1`, вставка после последнего — `a1 >= i2`.
        Тогда сторона выглядит как не тронувшая кусок, и её правка молча
        пропадает — так терялась группа форм, приписанная к слову, которое
        другая сторона вернула латиницей («[Плащ|Плаща|Плащей] из мозаики
        Шин Джеа» + «Плащ из мозаики Shing Jea»). Вставку берём, если её точка
        лежит в куске или на его границе.
        """
        j1 = j2 = None
