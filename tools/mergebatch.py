...
def _weld(base, ours, theirs):
    ...
    def span(ops, i1, i2):
        ...
        Чистая вставка занимает в базе НОЛЬ токенов, и обычная проверка
        пересечения её пропускает: `a2 <= i1` истинно уже на границе куска.
        Так терялась целая правка — сторона, надевшая на слово группу форм
        («Коробка» -> «[Коробка|Коробки|Коробок]»), отдавала кусок стороне,
        которая правила в этой же записи имя, и группа исчезала молча.
        Поэтому вставку берём по точке: она наша, если стоит внутри куска
        или на его границе. Двух кусков сразу это не касается — марки
        склеиваются в один кусок, когда сходятся в одной точке.
        """
        j1 = j2 = None
